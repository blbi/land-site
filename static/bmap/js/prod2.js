...
for (let i=0; i<prod_list.length; i++){
	geocoder.addressSearch(prod_list[i], function(result, status) {
	    ...
	    if (status === daum.maps.services.Status.OK) {
	        ...
	        daum.maps.event.addListener(markers[i], 'click', function() {
				document.getElementById(prod_list[i]).click(); 
			});
	        //markers.push(marker);
...
var imageSize = new daum.maps.Size(40, 45), // 마커이미지의 크기입니다
    imageOption = {offset: new daum.maps.Point(20, 45)}; // 마커이미지의 옵션입니다. 마커의 좌표와 일치시킬 이미지 안에서의 좌표를 설정합니다.
      
// 마커의 이미지정보를 가지고 있는 마커이미지를 생성합니다
var markerImage = new daum.maps.MarkerImage(imageSrc, imageSize, imageOption);

var red_marker = new daum.maps.Marker({
	image: markerImage
});

function show_loc(address){
	console.log(address);
	geocoder.addressSearch(address, function(result, status) {
	    ...
	    if (status === daum.maps.services.Status.OK) {
			...
			//*/
			daum.maps.event.addListener(red_marker, 'click', function() {
				document.getElementById(address).click(); 
			});
			red_marker.setMap(map);
	        map.setCenter(coords);
	    }
	});
}

function revert_marker(address){
	red_marker.setMap(null);
}
...
function searchInputAddr(){
	...
	geocoder.addressSearch($('input#search-box-input').val(), function(result, status) {
...
function toggle_detail(){
	if (document.getElementById('detail-search').style.display == "none"){
		document.getElementById('detail-search').style.display = "block";
	}
	else{
		document.getElementById('detail-search').style.display = "none";
	}
}

function showareavalue(val){
	document.getElementById('area-value').innerHTML = val+"평";
}
